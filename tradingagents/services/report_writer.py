"""Report writing helpers shared by CLI and non-interactive services."""
...
def find_latest_report(reports_dir: Path) -> Path | None:
    """Return the newest complete report under ``reports_dir``."""
    if not reports_dir.exists():
        return None
    candidates = [
        path / "complete_report.md"
        for path in reports_dir.iterdir()
        if path.is_dir() and (path / "complete_report.md").exists()
    ]
    if not candidates:
        return None
